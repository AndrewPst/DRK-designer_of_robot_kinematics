...
const QVector<ArgDescription_t> GTEST::_keys = {
    ArgDescription_t('T', ActionArgumentType_t::ARGTYPE_DOUBLE, "test"),
};
...
ActionExectionResult_t GTEST::startExecution(const IArgsCollection& args, const ExecutionEnivroment& )
{
    qDebug() << "Started";
    Arg_t arg(100);
    if(args.getValue('T', arg))
        _param = arg.getValue().toDouble();
    return ActionExectionResult_t::RESULT_IN_PROCESS;
}
